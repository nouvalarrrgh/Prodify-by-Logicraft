...
import { ArrowRight, Mail, Lock, User as UserIcon, Star, Zap, BookOpen } from 'lucide-react';
import { NekoMascotFull } from './NekoMascot';

// Hero student illustration (studying at laptop)
const StudentHeroIllustration = () => (
    <svg viewBox="0 0 380 340" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full max-w-[380px]">
        {/* Desk surface */}
        ...
        <path d="M50 60 L53 70 L63 70 L55 76 L58 86 L50 80 L42 86 L45 76 L37 70 L47 70 Z" fill="#FCD34D" opacity="0.6" />
        <path d="M340 50 L342 57 L349 57 L343 61 L346 68 L340 64 L334 68 L337 61 L331 57 L338 57 Z" fill="#A78BFA" opacity="0.7" />
        <circle cx="355" cy="90" r="5" fill="#4F46E5" opacity="0.3" />
        <circle cx="25" cy="140" r="8" fill="#10B981" opacity="0.2" />
        <circle cx="370" cy="150" r="4" fill="#F59E0B" opacity="0.4" />
    </svg>
);
    ...
    const handleSubmit = (e) => {
        ...
        if (isLogin) {
            if (formData.email && formData.password) {
                const user = { name: formData.email.split('@')[0], email: formData.email, token: 'mock-jwt-token-123' };
        ...
        <div className="min-h-[100dvh] flex flex-col lg:flex-row bg-slate-50 relative overflow-hidden">
            ...
            <div className="hidden lg:flex lg:flex-1 relative bg-gradient-to-br from-indigo-600 via-violet-600 to-purple-700 flex-col items-center justify-center p-12 overflow-hidden">
                ...
                <div className="relative z-10 text-center flex flex-col items-center gap-6 max-w-md">
                    ...
                    <div className="flex flex-wrap justify-center gap-3 mt-2">
                        {[
                            { icon: <BookOpen className="w-3.5 h-3.5" />, label: "ZenNotes" },
                            { icon: <Zap className="w-3.5 h-3.5" />, label: "Deep Focus" },
                            { icon: <Star className="w-3.5 h-3.5" />, label: "Habit Tracker" },
                        ].map((f) => (
            ...
            <div className="flex-1 flex items-center justify-center p-6 md:p-12 relative">
                ...
                <div className="w-full max-w-md z-10 animate-fade-in-up">
                    {/* Mobile only: small mascot + branding */}
                    ...
                    {/* Form card */}
                    <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl shadow-slate-200/60 border border-white p-8 md:p-10">
                        <div className="mb-8">
                            <h2 className="text-2xl font-black text-slate-800 tracking-tight">
                                {isLogin ? "Selamat Kembali 👋" : "Bergabung Sekarang 🚀"}
                            </h2>
                            <p className="text-slate-500 mt-1.5 text-sm font-medium">
                                {isLogin ? "Masuk untuk melanjutkan perjalanan produktivitasmu." : "Mulai kelola hidupmu lebih baik hari ini."}
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            {!isLogin && (
                                <div className="relative group">
                                    <input type="text" name="name" required placeholder="Nama Lengkap"
                                        className="w-full bg-slate-50 border border-slate-200 group-focus-within:border-indigo-400 rounded-2xl px-4 py-3.5 pl-11 outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all text-slate-800 placeholder:text-slate-400 font-medium"
                            ...
                            <button type="submit"
                                className="w-full group mt-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl px-4 py-3.5 font-bold transition-all shadow-lg shadow-indigo-500/25 hover:shadow-xl hover:shadow-indigo-500/30 hover:-translate-y-0.5 flex items-center justify-center gap-2 active:scale-95">
                                {isLogin ? "Masuk ke StuProd" : "Mulai Gratis"}
                    ...
                    {/* Bottom trust badges */}
                    <div className="flex justify-center gap-6 mt-6 text-xs text-slate-400 font-medium">
